...
class Visualization(object):
	...
	self.home_images = self.home_dir + "images" + "/" + args.project_name[0] + "/"
    ...
    def visualize(self):
	all_files = listdir(self.home_out)

	#print "PLIKI w output"
	#print all_files
        ...
        for traffic_load_file in traffic_load_files:
            traffic_load = persist_read(self.home_out + traffic_load_file, is_array = True)
        ...
        while len(traffic_load_files) > 0:
            ...
            if street_network_filename in street_network_files:
                ...
                self.street_network = persist_read(self.home_out + street_network_filename)
            ...
            if traffic_load_filename in traffic_load_files:
                ...
                traffic_load = persist_read(self.home_out + traffic_load_filename, is_array = True)
		...
		if not os.path.exists(self.home_images):
		    os.makedirs(self.home_images)

                street_network_image.save(self.home_images + "traffic_load_" + str(step) + ".png")
